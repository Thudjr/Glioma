...
# Parser Init
parser = argparse.ArgumentParser(description='glioma Training')
...
testset = glioma(cf.dataset_path + 'test', hp.targets, hp.test_num, hp.is_balanced, trans_test)
...
if args.resume:
    ...
    assert os.path.isdir(cf.checkpint_path), 'Error: no checkpoint directory found!'
    checkpoint = torch.load(os.path.join(cf.checkpint_path,'ckpt.t7'))
    net = checkpoint['net']
...
def train(epoch, wrong_save=False):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        sm = nn.Softmax(dim=1)
        probs = sm(outputs)
        predicts = torch.argmax(probs, dim=1, keepdim=False)
        correct += predicts.data.eq(targets.data).cpu().sum()

        train_loss += loss.data.item()
        total += targets.size(0)

        progressBar(batch_idx, len(trainloader), 'Loss: %.3f | Acc: %.3f%% (%d/%d)'
...
def valid(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(valloader):
        ...
        progressBar(batch_idx, len(valloader), 'Loss: %.3f | Acc: %.3f%% (%d/%d)'
                     % (valid_loss / (batch_idx + 1), 100. * correct / total, correct, total))

    correct, tp, tn, fp, fn, recall, precision, specificity, f1_score, auc, threshold = stats(outputs_list,
                                                                                              targets_list)
    acc = correct / total
    THRESHOLD = threshold
    ...
    if auc > BEST_AUC:
        ...
        torch.save(state, cf.checkpint_path + 'ckpt.t7')
...
def draw_graph():
    ...
    if len(CUR_TRA_ACC) != 0:
        ...
        plt.title('glioma DenseNet/Train acc')
        plt.xlabel('epoch')
    ...
    plt.title('glioma DenseNet/Val acc')
    plt.xlabel('epoch')
    ...
    plt.title('glioma DenseNet/Loss')
    plt.xlabel('epoch')
    plt.ylabel('loss')
